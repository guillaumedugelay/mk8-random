...
export default function FinalRanking({ ranking, scores, playersList, luckyPlayer, wantedPlayer, onRestart }) {
  useEffect(() => {
          ...
          return (
            <div key={joueur} className={`final-ranking-row rank-pos-${Math.min(idx + 1, 4)}`}>
              ...
              <div className="rank-info">
                <span className="rank-name">{joueur} <DestinyBadge joueur={joueur} luckyPlayer={luckyPlayer} wantedPlayer={wantedPlayer} /></span>
                {citationMap[joueur] && (
